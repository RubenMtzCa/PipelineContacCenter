        ...
        // --------------------------------------------------------------------------------
        const location = scriptingEnums.LOCATIONS.prod_eu_west_1;
        ...
        function doWork(scriptSession) {
            ...
            return scriptingFlowFactory.createFlowInboundCallAsync(flowName, flowDescription, scriptingLanguages
                ...
                function (archInboundCallFlow) {
                    ...
                    archInboundCallFlow.settingsSpeechRec.asrCompanyDir = scriptingEnums.SPEECH_REC_COMPANY_MODES
                        .none;
                    ...
                    const decisionAction = scriptingActionFactory.addActionDecision(startupTask,
                        'greater than check');
                    ...
                    // used when creating constious things like menus, actions, tasks or even flows themselves.
                    ...
                    return archInboundCallFlow.validateAsync()
                        .then(function (validationResults) {
                            ...
                            if (validationResults.hasErrors) {
                                scriptingLogger.logError(
                                    'There is at least one validation error in the created flow.  Not publishing.'
                                    );
                            } else if (validationResults.hasWarnings) {
                                scriptingLogger.logWarning(
                                    'There is at least one validation warning in the created flow.  Not publishing.'
                                    );
                            } else {

                                scriptingLogger.logNote(
                                    'The flow has no validation errors or warnings.  Time to publish it.');
                                ...
                                return archInboundCallFlow.publishAsync()
                                    .then(function () {
                                        ...
                                        scriptingLogger.logNote('The flow \'' + archInboundCallFlow.name +
                                            '\' is now published in and available in Architect.');
